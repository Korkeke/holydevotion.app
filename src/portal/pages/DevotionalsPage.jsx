...
export default function DevotionalsPage() {
  ...
  const showRightPanel = aiOpen || selectedItem;
    ...
    <div style={{ padding: "24px 32px 48px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <Button onClick={openAiPanel} style={aiOpen ? { background: C.accent + "18", color: C.accent, border: `1px solid ${C.accent}40` } : {}}>
            AI Assist
          </Button>
      ...
      {/* Content area: table + right panel */}
      {filtered.length === 0 && !aiOpen ? (
        <Card>
          <EmptyState
            emoji="📖"
        ...
        <div style={{ display: "grid", gridTemplateColumns: showRightPanel ? "1fr 420px" : "1fr", gap: 20 }}>
          ...
          {filtered.length > 0 ? (
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {filtered.map((row, idx) => (
                    <tr
                      key={row.id || idx}
                      onClick={() => { setSelected(idx); setAiOpen(false); }}
                      style={{
                        cursor: "pointer",
                        borderBottom: idx < filtered.length - 1 ? "1px solid #f0ebe3" : "none",
                        background: selected === idx && !aiOpen ? "#faf6ee" : "transparent",
            ...
            <Card>
              <EmptyState
                emoji="📖"
                title="No devotionals yet"
                desc="Use AI Assist on the right to generate your first devotional."
              />
            </Card>
          )}
          ...
          {aiOpen && (
            <Card style={{ position: "relative", alignSelf: "start" }}>
              ...
              <SectionLabel style={{ marginBottom: 12 }}>AI Assist</SectionLabel>
              ...
              {aiLoading && (
                <div style={{ display: "flex", justifyContent: "center", padding: "20px 0" }}>
                  <div style={{ width: 24, height: 24, border: `2px solid ${C.accent}`, borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
                </div>
              )}

              {/* Single result - editable preview */}
              {aiResult && !aiLoading && (
                <div style={{ borderTop: "1px solid #ece7dd", paddingTop: 14 }}>
